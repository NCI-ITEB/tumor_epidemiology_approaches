...
* `--dependency afterany:$jobid1`: This sets a dependency on a previous job with ID `$jobid1`, meaning that this job will not start until the previous one has completed. 
*(note: if you want the job to run only if the previous job has completed without errors, you can change `afterany` to `afterok`. See the [Biowulf documentation on dependencies](https://hpc.nih.gov/docs/userguide.html#depend) for more details)*
